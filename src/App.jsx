...
function SupplierProductForm({ initial, products, categories, onSave, onCancel }) {
  const symbol = useCurrencySymbol();
  ...
  return (
    <div className="rounded-2xl border p-4 mb-3 gb-slide-up" style={{ borderColor: "var(--line)", background: "var(--card)" }}>
      <div className="flex flex-col gap-2.5">
        ...
        />
        <select className="gb-focus rounded-xl px-3 py-2 text-sm border" style={{ borderColor: "var(--line)" }} value={category} onChange={(e) => setCategory(e.target.value)}>
          {categories.map((c) => <option key={c.id} value={c.id}>{c.label}</option>)}
        </select>
...
function SupplierProductsModal({ supplier, products, categories, supplierProducts, onSave, onDelete, onClose }) {
  const fmt = useFmt();
  const [adding, setAdding] = useState(false);
  const [editing, setEditing] = useState(null);
    ...
    <div className="fixed inset-0 z-50 flex items-end no-print">
      ...
      <div className="relative w-full rounded-t-3xl gb-slide-up max-h-[85vh] flex flex-col" style={{ background: "var(--card)" }}>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto gb-scroll p-5 pt-3">
          {adding && <SupplierProductForm products={products} categories={categories} onSave={save} onCancel={() => setAdding(false)} />}
          <div className="flex flex-col gap-2.5">
            {filtered.map((sp) => editing === sp.id ? (
              <SupplierProductForm key={sp.id} initial={sp} products={products} categories={categories} onSave={save} onCancel={() => setEditing(null)} />
            ) : (
...
function SuppliersSection({ suppliers, saveSuppliers, expenses, saveExpenses, products, saveProducts, categories, movements, saveMovements, orders, saveOrders, supplierProducts, saveSupplierProducts, pushToast }) {
  const fmt = useFmt();
  const [editing, setEditing] = useState(null);
  const [adding, setAdding] = useState(false);
    ...
    order.items.forEach((i) => {
      ...
      if (existing) {
        ...
        const newProduct = {
          ...
          minStock: i.crateSize, unit: "bouteille", favorite: false,
        };
...
function AdminScreen({
  ...
  return (
    <div className="px-4 pt-4 pb-28">
      ...
      {section === "fournisseurs" && <SuppliersSection suppliers={suppliers} saveSuppliers={saveSuppliers} expenses={expenses} saveExpenses={saveExpenses} products={products} saveProducts={saveProducts} categories={categories} movements={movements} saveMovements={saveMovements} orders={orders} saveOrders={saveOrders} supplierProducts={supplierProducts} saveSupplierProducts={saveSupplierProducts} pushToast={pushToast} />}
