...
async function loadShopData(shopId) {
  const [products, sales, vendors, suppliers, expenses, categories, movements, inventories, clients, orders] = await Promise.all([
  ...
  return { products, sales, vendors, suppliers, expenses, categories, movements, inventories, clients, orders };
}
...
function PurchaseOrderModal({ supplier, products, onCreate, onClose }) {
  const fmt = useFmt();
  ...
  const lineItems = Object.entries(selected).map(([id, qty]) => {
    const p = products.find((x) => x.id === id);
    return p ? { productId: p.id, productName: p.name, unit: p.unit, qty } : null;
  }).filter(Boolean);
  const lines = lineItems.map((l) => `- ${l.productName} : ${l.qty} ${l.unit}${l.qty > 1 ? "s" : ""}`);
...
function PendingOrderModal({ order, supplier, products, onUpdate, onValidate, onClose, pushToast }) {
  const fmt = useFmt();
  ...
  const total = items.reduce((sum, i) => {
    ...
    return sum + (p ? Number(p.costPrice || 0) * i.qty : 0);
  }, 0);
            ...
            return (
              <div key={i.productId} className="rounded-xl p-2.5 border flex items-center gap-2.5" style={{ borderColor: "var(--line)" }}>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[10px] opacity-50">{i.qty} {i.unit}{i.qty > 1 ? "s" : ""}{p ? ` · ${fmt(Number(p.costPrice || 0) * i.qty)}` : ""}</div>
                </div>
...
function SuppliersSection({ suppliers, saveSuppliers, expenses, saveExpenses, products, saveProducts, movements, saveMovements, orders, saveOrders, pushToast }) {
  const fmt = useFmt();
  const [editing, setEditing] = useState(null);
  const [adding, setAdding] = useState(false);
  ...
  // produits restants (après retrait des articles manquants) sont réceptionnés :
  // le stock augmente, une dépense est enregistrée, et la commande passe "reçue".
  const validateOrder = (order) => {
    ...
    const receiveMovements = order.items.map((i) => {
      ...
      return { id: uid(), date: new Date().toISOString(), productId: i.productId, productName: i.productName, type: "livraison", delta: i.qty, before, after: before + i.qty, author: "Administrateur", note: "Commande fournisseur validée" };
    });
    ...
    pushToast("Commande validée — stock mis à jour et dépense enregistrée", "ok");
  };
  ...
  return (
    <div>
      ...
      {orderFor && <PurchaseOrderModal supplier={orderFor} products={products} onCreate={createOrder} onClose={() => setOrderFor(null)} />}
      {openOrder && (
        <PendingOrderModal
          order={openOrder}
...
function AdminScreen({
  ...
  return (
    <div className="px-4 pt-4 pb-28">
      ...
      {section === "fournisseurs" && <SuppliersSection suppliers={suppliers} saveSuppliers={saveSuppliers} expenses={expenses} saveExpenses={saveExpenses} products={products} saveProducts={saveProducts} movements={movements} saveMovements={saveMovements} orders={orders} saveOrders={saveOrders} pushToast={pushToast} />}
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setProducts(data.products); setSales(data.sales); setVendors(data.vendors); setSuppliers(data.suppliers); setExpenses(data.expenses); setCategories(data.categories); setMovements(data.movements); setInventories(data.inventories); setClients(data.clients); setOrders(data.orders);
      let storedRole = null;
  ...
  const handleOnboardingComplete = async (shopMeta, vendor) => {
    ...
    setProducts(SEED_PRODUCTS); setSales([]); setVendors([vendor]); setSuppliers(SEED_SUPPLIERS); setExpenses([]); setCategories(SEED_CATEGORIES); setMovements([]); setInventories([]); setClients([]); setOrders([]);
    if (newShop.backendLinked) {
      const keys = ["shopMeta", "vendors", "products", "sales", "categories", "suppliers", "expenses", "movements", "inventories", "clients", "orders", "shopLicense"];
      ...
      const initialValues = {
        ...
        expenses: [], movements: [], inventories: [], clients: [], orders: [],
        shopLicense: license,
      };
      try {
  ...
  const handleJoinShopComplete = async (shopObj, pulled, autoRole, autoVendorName, shopLicense) => {
    ...
    setExpenses(expensesData); setCategories(categoriesData); setMovements(movementsData); setInventories(inventoriesData); setClients(clientsData); setOrders(ordersData);
    if (shopLicense) {
  ...
  const handleDeleteShop = async (shopId) => {
    ...
    await Promise.all([
      ...
      safeDelete(`suppliers:${shopId}`), safeDelete(`expenses:${shopId}`), safeDelete(`categories:${shopId}`), safeDelete(`movements:${shopId}`), safeDelete(`inventories:${shopId}`), safeDelete(`clients:${shopId}`), safeDelete(`orders:${shopId}`),
    ]);
  ...
  const handleRestoreBackup = (bundle) => {
    ...
    setExpenses(next.expenses); setMovements(next.movements); setInventories(next.inventories); setClients(next.clients); setVendors(next.vendors); setOrders(next.orders);
