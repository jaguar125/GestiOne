  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end no-print">
      ...
      <div className="relative w-full rounded-t-3xl p-5 gb-slide-up max-h-[85vh] overflow-y-auto gb-scroll" style={{ background: "var(--card)" }}>
        ...
        <p className="text-xs opacity-50 mb-4">{supplier?.name} · commandée le {new Date(order.date).toLocaleDateString("fr-FR")}{order.receivedDate ? ` · reçue le ${new Date(order.receivedDate).toLocaleDateString("fr-FR")}` : ""}</p>
        <div className="flex flex-col gap-2 mb-4">
      ...
      <div className="relative w-full rounded-t-3xl p-5 gb-slide-up max-h-[85vh] overflow-y-auto gb-scroll" style={{ background: "var(--card)" }}>
        ...
        <div className="flex flex-col gap-2">
          {sorted.map((o) => {
            ...
            return (
              <button key={o.id} onClick={() => onOpenOrder(o)} className="gb-focus rounded-xl p-3 border flex items-center justify-between text-left" style={{ borderColor: "var(--line)" }}>
                <div className="min-w-0">
                  <div className="text-xs font-medium">{new Date(o.date).toLocaleDateString("fr-FR")} · {o.items.length} article{o.items.length > 1 ? "s" : ""}</div>
                  <div className="text-[10px] opacity-50 font-mono mt-0.5">{fmt(total)}</div>
                </div>
                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full text-white shrink-0" style={{ background: o.status === "received" ? "#3B6D11" : "var(--cap)" }}>
