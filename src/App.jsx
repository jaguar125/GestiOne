  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end no-print">
      ...
      <div className="relative w-full rounded-t-3xl p-5 gb-slide-up max-h-[85vh] overflow-y-auto gb-scroll" style={{ background: "var(--card)" }}>
        ...
        <button onClick={() => { setAdding(true); setEditing(null); }} className="gb-focus w-full flex items-center justify-center gap-1.5 rounded-xl py-2.5 text-sm font-semibold text-white mb-3" style={{ background: "var(--glass)" }}><Plus size={15} /> Lier un produit</button>
        ...
        <div className="flex flex-col gap-2.5">
          {list.map((sp) => editing === sp.id ? (
            ...
            <div key={sp.id} className="rounded-2xl p-3.5 border" style={{ borderColor: "var(--line)", background: "var(--card)" }}>
              <div className="flex items-center justify-between mb-2.5">
                <p className="text-sm font-semibold truncate">{sp.productName}</p>
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => { setEditing(sp.id); setAdding(false); }} className="gb-focus w-7 h-7 rounded-full flex items-center justify-center" style={{ background: "var(--paper-dim)" }}><Pencil size={12} /></button>
                  <button onClick={() => onDelete(sp.id)} className="gb-focus w-7 h-7 rounded-full flex items-center justify-center" style={{ background: "var(--paper-dim)" }}><Trash2 size={12} color="var(--danger)" /></button>
                </div>
              </div>
              <div className="flex gap-2 mb-2.5">
                <div className="flex-1 rounded-xl px-2.5 py-2" style={{ background: "var(--paper-dim)" }}>
                  <p className="text-[9px] opacity-50">Quantité par casier</p>
                  <p className="text-sm font-mono font-semibold mt-0.5">{sp.crateSize} bouteilles</p>
                </div>
                <div className="flex-1 rounded-xl px-2.5 py-2" style={{ background: "var(--paper-dim)" }}>
                  <p className="text-[9px] opacity-50">Prix du casier</p>
                  <p className="text-sm font-mono font-semibold mt-0.5">{fmt(sp.cratePrice)}</p>
                </div>
              </div>
              <div className="flex items-center justify-between pt-2.5" style={{ borderTop: "1px solid var(--line)" }}>
                <span className="text-[11px] opacity-50">Coût par bouteille (auto)</span>
                <span className="font-mono text-xs font-semibold" style={{ color: "#0F6E56" }}>{fmt(sp.crateSize > 0 ? Math.round(sp.cratePrice / sp.crateSize) : 0)}</span>
              </div>
            </div>
          ))}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end no-print">
      ...
      <div className="relative w-full rounded-t-3xl p-5 gb-slide-up max-h-[85vh] overflow-y-auto gb-scroll" style={{ background: "var(--card)" }}>
        ...
        <div className="flex flex-col gap-2.5">
          {sorted.map((o) => {
            const total = o.items.reduce((sum, i) => sum + i.crates * i.cratePrice, 0);
            const received = o.status === "received";
            return (
              <button key={o.id} onClick={() => onOpenOrder(o)} className="gb-focus rounded-2xl p-3.5 border text-left" style={{ borderColor: "var(--line)", background: "var(--card)" }}>
                <div className="flex items-center justify-between">
                ...
                <p className="text-xs opacity-50 mt-0.5">N° {o.id.slice(0, 6).toUpperCase()} · {o.items.map((i) => `${i.crates} casier${i.crates > 1 ? "s" : ""} de ${i.crateSize}`).join(" · ")}</p>
                <div className="flex items-center justify-between mt-2.5 pt-2.5" style={{ borderTop: "1px solid var(--line)" }}>
                  ...
                  <span className="font-mono text-sm font-semibold">{fmt(total)}</span>
                </div>
              </button>
            );
          })}
...
function SuppliersSection({ suppliers, saveSuppliers, expenses, saveExpenses, products, saveProducts, movements, saveMovements, orders, saveOrders, supplierProducts, saveSupplierProducts, pushToast }) {
  ...
  const validateOrder = (order) => {
    ...
    const receiveMovements = order.items.filter((i) => i.productId).map((i) => {
