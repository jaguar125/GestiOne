...
function ActivationScreen({ onActivate, onStartTrial, trialUsed, pushToast, registeredAdmin, onReconnectInstead, hasSavedShop, savedShopName, onResume }) {
  return (
    <div className="min-h-full flex flex-col items-center justify-center px-6 py-10 relative" style={{ background: "var(--glass)" }}>
      <div className="mb-10 text-center gb-slide-up">
        <svg width="96" height="96" viewBox="0 0 512 512" className="mx-auto mb-4" style={{ filter: "drop-shadow(0 6px 16px rgba(0,0,0,0.25))" }}>
          <defs>
        ...
        <h1 className="font-display font-bold text-2xl text-white tracking-tight">
          Case <span style={{ color: "var(--cap)" }}>&amp;</span> Comptoir
        </h1>
        <p className="text-white/50 text-xs mt-1 tracking-wide">GESTION DE STOCK &amp; VENTES</p>
      </div>

      {hasSavedShop && (
        <button onClick={onResume} className="gb-focus w-full max-w-xs rounded-2xl py-3.5 font-semibold text-sm mb-3 active:scale-[0.98] transition-transform" style={{ background: "var(--cap)", color: "var(--glass)" }}>
          ↩ Retourner à "{savedShopName}"
        </button>
      )}

      {!trialUsed && (
        <button onClick={onStartTrial} className="gb-focus w-full max-w-xs rounded-2xl py-3.5 font-semibold text-sm text-white mb-3" style={{ background: "var(--glass-light)" }}>
          Démarrer l'essai gratuit ({TRIAL_DAYS} jours)
        </button>
      )}

      <button onClick={onReconnectInstead} className={`gb-focus w-full max-w-xs rounded-2xl py-3.5 font-semibold text-sm active:scale-[0.98] transition-transform ${hasSavedShop ? "text-white" : ""}`} style={hasSavedShop ? { background: "var(--glass-light)" } : { background: "var(--cap)", color: "var(--glass)" }}>
        Tu avais déjà une boutique ? Se reconnecter
      </button>
    </div>
  );
}
...
function OnboardingScreen({ shops, onComplete, onJoinShop, pushToast, initialMode, onCancel }) {
...
export default function App() {
  ...
  const [bypassActivationForReconnect, setBypassActivationForReconnect] = useState(false);
  ...
  const handleJoinShopComplete = async (shopObj, pulled, autoRole, autoVendorName, shopLicense) => {
    ...
    setBypassActivationForReconnect(false);
  ...
  return (
    <div className={`gb-root min-h-screen flex justify-center${isDark ? " gb-dark" : ""}`} style={themeVars}>
      ...
      <div className="w-full max-w-[430px] sm:max-w-[600px] lg:max-w-[880px] xl:max-w-[1100px] min-h-screen relative" style={{ background: "var(--paper)" }}>
        ...
        {bypassActivationForReconnect ? (
          <OnboardingScreen shops={[]} initialMode="reconnect" onComplete={handleOnboardingComplete} onJoinShop={handleJoinShopComplete} pushToast={pushToast} onCancel={() => setBypassActivationForReconnect(false)} />
        ) : homeScreenActive ? (
          <ActivationScreen
            ...
            trialUsed={trialUsed}
            pushToast={pushToast}
            onReconnectInstead={() => setBypassActivationForReconnect(true)}
            hasSavedShop={shops.length > 0}
            savedShopName={shop?.name || shops[0]?.name}
            onResume={() => setHomeScreenActive(false)}
          />
        ) : licenseStatus === "none" ? (
          <ActivationScreen onActivate={handleActivateLicense} onStartTrial={handleStartTrial} trialUsed={trialUsed} pushToast={pushToast} onReconnectInstead={() => setBypassActivationForReconnect(true)} />
        ) : shops.length === 0 ? (
          <OnboardingScreen shops={shops} onComplete={handleOnboardingComplete} onJoinShop={handleJoinShopComplete} pushToast={pushToast} />
        ) : shop && (
