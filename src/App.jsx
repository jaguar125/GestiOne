...
import {
  ...
  Zap, Rocket, Crown, Building2, Infinity, Barcode, Banknote, Smartphone,
} from "lucide-react";
...
const LICENSE_PLANS_PRICING = [
  { id: "1m", name: "Essentiel", duration: "1 mois", price: 5000, oldPrice: 15000, icon: "Zap", iconBg: "#EAF3DE", iconColor: "#27500A", borderColor: "#4C7A2E" },
  { id: "3m", name: "Avancé", duration: "3 mois", price: 13000, oldPrice: 50000, icon: "Rocket", iconBg: "#E6F1FB", iconColor: "#0C447C", borderColor: "#1E6FB8", popular: true },
  { id: "6m", name: "Professionnel", duration: "6 mois", price: 25000, oldPrice: 150000, icon: "Crown", iconBg: "#F0EDFE", iconColor: "#3C3489", borderColor: "#6B5FC4" },
  { id: "12m", name: "Entreprise", duration: "12 mois", price: 45000, oldPrice: 250000, icon: "Building2", iconBg: "#FAEEDA", iconColor: "#633806", borderColor: "#B8863A" },
    ...
    <div className="min-h-full px-5 py-8" style={{ background: "var(--glass)" }}>
      ...
      <div className="flex flex-col gap-3">
        {LICENSE_PLANS_PRICING.map((p) => {
          ...
          if (p.id === "lifetime") {
            return (
              <div key={p.id} className="rounded-2xl p-4" style={{ background: "linear-gradient(135deg, var(--cap), var(--cap-dim, var(--cap)))", border: "1px solid rgba(255,255,255,0.15)" }}>
                <div className="flex items-center gap-2.5 mb-2">
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: "rgba(255,255,255,0.18)" }}><PlanIcon size={18} color="#fff" /></div>
                  <div className="flex-1">
                    <p className="text-white font-display font-bold text-base">{p.name}</p>
                    <p className="text-white/60 text-[11px]">{p.duration}</p>
                  </div>
                </div>
                <p className="text-white/70 text-xs mb-3">Accès à vie — tarif sur demande</p>
                <button onClick={() => setOrdering(p)} className="gb-focus w-full rounded-xl py-2.5 text-sm font-semibold text-white" style={{ background: "rgba(255,255,255,0.18)", border: "1px solid rgba(255,255,255,0.3)" }}>
          ...
          return (
            <div key={p.id} className="relative rounded-2xl p-4" style={{ background: "var(--glass-light)", border: `2px solid ${p.borderColor}` }}>
              {p.popular && (
                <span className="absolute -top-2.5 left-4 text-[10px] font-semibold px-2.5 py-0.5 rounded-full" style={{ background: "var(--cap)", color: "var(--glass)" }}>Le plus choisi</span>
              )}
              <div className="flex items-center gap-2.5 mb-1 mt-0.5">
                <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: p.iconBg }}><PlanIcon size={18} color={p.iconColor} /></div>
                <div className="flex-1">
                  <p className="text-white font-display font-bold text-base">{p.name}</p>
                  <p className="text-white/50 text-[11px]">{p.duration}</p>
                </div>
              </div>
              ...
              <button onClick={() => setOrdering(p)} className="gb-focus w-full rounded-xl py-2.5 text-sm font-semibold" style={p.popular ? { background: "var(--cap)", color: "var(--glass)" } : { background: "rgba(255,255,255,0.12)", color: "#fff", border: "1px solid rgba(255,255,255,0.25)" }}>
                Commander
              </button>
            </div>
          );
    ...
    <div className="px-4 pt-4 pb-28">
      ...
      <div className="flex flex-col gap-2.5">
        {sorted.map((s) => {
          ...
          return (
          <div key={s.id} className="rounded-2xl overflow-hidden" style={{ border: unpaid ? "1.5px solid var(--danger)" : "1px solid var(--line)", background: "var(--card)", boxShadow: unpaid ? "0 4px 14px rgba(163,45,45,0.18)" : "0 4px 14px rgba(0,0,0,0.08)" }}>
            <button onClick={() => setOpen(open === s.id ? null : s.id)} className="gb-focus w-full flex items-center gap-3 p-3.5">
              ...
              <div className="flex-1 text-left min-w-0">
                ...
                <div className="text-xs opacity-50 flex items-center gap-1.5 mt-0.5 flex-wrap">
                  ...
                  <span className="px-1.5 py-0.5 rounded-full text-[9px] font-semibold text-white" style={{ background: PAYMENT_COLORS[s.paymentMethod] }}>
                    {PAYMENT_LABELS[s.paymentMethod]}{unpaid ? " · impayé" : ""}
                  </span>
                </div>
    ...
    <div className="rounded-2xl border p-4 mb-3 gb-slide-up" style={{ borderColor: "var(--line)", background: "var(--card)" }}>
      ...
      <div className="flex items-center gap-3 mb-3">
        <div className="w-16 h-16 rounded-2xl overflow-hidden shrink-0 flex items-center justify-center" style={{ background: "var(--paper-dim)" }}>
          {f.image ? <img src={f.image} alt="" className="w-full h-full object-cover" /> : <CategoryIcon cat={f.category} categories={categories} size={22} />}
        </div>
        <div className="flex-1 flex gap-2">
          <button onClick={() => cameraInputRef.current?.click()} className="gb-focus flex-1 flex items-center justify-center gap-1.5 rounded-xl py-2 text-xs font-semibold" style={{ background: "var(--paper-dim)" }}><Camera size={13} /> Photo</button>
          <button onClick={() => galleryInputRef.current?.click()} className="gb-focus flex-1 flex items-center justify-center gap-1.5 rounded-xl py-2 text-xs font-semibold" style={{ background: "var(--paper-dim)" }}><Layers size={13} /> Galerie</button>
          {f.image && <button onClick={() => set("image", null)} className="gb-focus w-8 h-8 rounded-xl flex items-center justify-center shrink-0" style={{ background: "var(--paper-dim)" }} aria-label="Retirer la photo"><Trash2 size={13} color="var(--danger)" /></button>}
        </div>
      </div>
...
function InventorySection({ products, saveProducts, categories, movements, saveMovements, inventories, saveInventories, author, pushToast, pushNotification }) {
...
function SuppliersSection({ suppliers, saveSuppliers, expenses, saveExpenses, products, saveProducts, categories, saveCategories, movements, saveMovements, orders, saveOrders, supplierProducts, saveSupplierProducts, pushToast, pushNotification }) {
  const fmt = useFmt();
  const [editing, setEditing] = useState(null);
  const [adding, setAdding] = useState(false);
...
function ExpensesSection({ expenses, saveExpenses, suppliers, pushNotification }) {
  const fmt = useFmt();
  const [adding, setAdding] = useState(false);
  const add = (e) => { saveExpenses([...expenses, e]); setAdding(false); pushNotification?.({ type: "expense", label: e.label, amount: e.amount, author: e.author }); };
  ...
  return (
    <div>
      ...
      {adding && <ExpenseForm onSave={add} onCancel={() => setAdding(false)} suppliers={suppliers} author="Administrateur" />}
      <div className="flex flex-col gap-2.5">
        {sorted.map((e) => {
          const supplier = suppliers.find((s) => s.id === e.supplierId);
          return (
...
function AdminScreen({
  ...
  menuOpen, setMenuOpen, pushNotification,
}) {
  ...
  return (
    <div className="px-4 pt-4 pb-28">
      ...
      {section === "inventaire" && <InventorySection products={products} saveProducts={saveProducts} categories={categories} movements={movements} saveMovements={saveMovements} inventories={inventories} saveInventories={saveInventories} author="Administrateur" pushToast={pushToast} pushNotification={pushNotification} />}
      {section === "produits" && <ProductsSection products={products} saveProducts={saveProducts} categories={categories} movements={movements} saveMovements={saveMovements} author="Administrateur" pushToast={pushToast} pushNotification={pushNotification} />}
      ...
      {section === "fournisseurs" && <SuppliersSection suppliers={suppliers} saveSuppliers={saveSuppliers} expenses={expenses} saveExpenses={saveExpenses} products={products} saveProducts={saveProducts} categories={categories} saveCategories={saveCategories} movements={movements} saveMovements={saveMovements} orders={orders} saveOrders={saveOrders} supplierProducts={supplierProducts} saveSupplierProducts={saveSupplierProducts} pushToast={pushToast} pushNotification={pushNotification} />}
      {section === "depenses" && <ExpensesSection expenses={expenses} saveExpenses={saveExpenses} suppliers={suppliers} pushNotification={pushNotification} />}
...
function SupportChatWidget({ shop }) {
  ...
  return (
    <>
      {!open && (
        <button onClick={() => setOpen(true)} className="gb-focus fixed right-4 z-40 w-14 h-14 rounded-full flex items-center justify-center" style={{ background: "var(--glass)", boxShadow: "0 4px 14px rgba(0,0,0,0.3)", bottom: "calc(6rem + env(safe-area-inset-bottom))" }} aria-label="Assistance">
...
const TABS = {
  vendeur: [{ id: "sell", label: "Vendre", Icon: ScanLine }, { id: "stock", label: "Stock", Icon: Boxes }, { id: "credits", label: "Crédits", Icon: CreditCard }, { id: "history", label: "Historique", Icon: History }, { id: "expenses", label: "Dépenses", Icon: Wallet }],
...
export default function App() {
  ...
  const handleSettleCredit = (saleId) => {
    ...
    pushNotification({ type: "credit_settled", clientName: settled?.clientName || settled?.vendor, total: settled?.total, by: paidBy });
    return settled;
  };
        ...
        {homeScreenActive ? (
          ...
          <LanguageContext.Provider value={shop.language || "fr"}>
            {!role ? (
              ...
              <>
                <div className="px-4 pt-4 pb-2 flex items-center justify-between sticky top-0 z-20 no-print" style={{ background: "var(--paper)", boxShadow: "0 4px 10px -6px rgba(0,0,0,0.18)" }}>
                  <div>
                ...
                {view === "admin" && role === "admin" && (
                  <AdminScreen
                    ...
                    menuOpen={adminMenuOpen} setMenuOpen={setAdminMenuOpen} pushNotification={pushNotification}
                  />
                )}
