...
function SupplierProductForm({ initial, products, onSave, onCancel }) {
  const symbol = useCurrencySymbol();
  ...
  return (
    <div className="rounded-2xl border p-4 mb-3 gb-slide-up" style={{ borderColor: "var(--line)", background: "var(--card)" }}>
      <div className="flex flex-col gap-2.5">
        <select className="gb-focus rounded-xl px-3 py-2 text-sm border" style={{ borderColor: "var(--line)" }} value={productId} onChange={(e) => setProductId(e.target.value)}>
...
function SupplierProductsModal({ supplier, products, supplierProducts, onSave, onDelete, onClose }) {
  const fmt = useFmt();
  const [adding, setAdding] = useState(false);
  const [editing, setEditing] = useState(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end no-print">
      ...
      <div className="relative w-full rounded-t-3xl p-5 gb-slide-up max-h-[85vh] overflow-y-auto gb-scroll" style={{ background: "var(--card)" }}>
        ...
        <p className="text-xs opacity-50 mb-4">Prix par casier, tel que vendu par ce fournisseur.</p>
        <button onClick={() => { setAdding(true); setEditing(null); }} className="gb-focus w-full flex items-center justify-center gap-1.5 rounded-xl py-2.5 text-sm font-semibold text-white mb-3" style={{ background: "var(--glass)" }}><Plus size={15} /> Ajouter un produit</button>
        {adding && <SupplierProductForm products={products} onSave={save} onCancel={() => setAdding(false)} />}
        <div className="flex flex-col gap-2">
          {list.map((sp) => editing === sp.id ? (
            <SupplierProductForm key={sp.id} initial={sp} products={products} onSave={save} onCancel={() => setEditing(null)} />
          ) : (
            <div key={sp.id} className="rounded-xl p-2.5 border flex items-center gap-2.5" style={{ borderColor: "var(--line)" }}>
              <div className="flex-1 min-w-0">
                <div className="text-xs font-medium truncate">{sp.productName}</div>
                <div className="text-[10px] opacity-50">Casier de {sp.crateSize} · {fmt(sp.cratePrice)}</div>
              </div>
              <button onClick={() => { setEditing(sp.id); setAdding(false); }} className="gb-focus w-8 h-8 rounded-full flex items-center justify-center shrink-0" style={{ background: "var(--paper-dim)" }}><Pencil size={13} /></button>
              <button onClick={() => onDelete(sp.id)} className="gb-focus w-8 h-8 rounded-full flex items-center justify-center shrink-0" style={{ background: "var(--paper-dim)" }}><Trash2 size={13} color="var(--danger)" /></button>
            </div>
          ))}
          {list.length === 0 && !adding && <p className="text-xs opacity-50 text-center py-6">Aucun produit lié à ce fournisseur pour l'instant.</p>}
        </div>
      </div>
    </div>
  );
...
function PurchaseOrderModal({ supplier, supplierProducts, onCreate, onClose }) {
  ...
  const lineItems = Object.entries(crates).map(([id, n]) => {
    ...
    return sp ? { productId: sp.productId, productName: sp.productName, crateSize: sp.crateSize, cratePrice: sp.cratePrice, crates: n } : null;
  }).filter(Boolean);
...
function PendingOrderModal({ order, supplier, onUpdate, onValidate, onClose, pushToast }) {
  ...
  const removeLine = (productId) => setItems((prev) => prev.filter((i) => i.productId !== productId));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end no-print">
      ...
      <div className="relative w-full rounded-t-3xl p-5 gb-slide-up max-h-[85vh] overflow-y-auto gb-scroll" style={{ background: "var(--card)" }}>
        ...
        <p className="text-xs opacity-50 mb-4">{supplier?.name} · {new Date(order.date).toLocaleDateString("fr-FR")}</p>
        ...
        <div className="flex flex-col gap-2 mb-4">
          {items.map((i) => (
            <div key={i.productId} className="rounded-xl p-2.5 border flex items-center gap-2.5" style={{ borderColor: "var(--line)" }}>
              <div className="flex-1 min-w-0">
              ...
              <button onClick={() => removeLine(i.productId)} className="gb-focus w-8 h-8 rounded-full flex items-center justify-center shrink-0" style={{ background: "var(--paper-dim)" }} title="Produit manquant chez le fournisseur">
...
function SuppliersSection({ suppliers, saveSuppliers, expenses, saveExpenses, products, saveProducts, movements, saveMovements, orders, saveOrders, supplierProducts, saveSupplierProducts, pushToast }) {
  const fmt = useFmt();
  const [editing, setEditing] = useState(null);
  const [adding, setAdding] = useState(false);
  ...
  const saveSupplierProduct = (entry) => { const exists = allSupplierProducts.some((x) => x.id === entry.id); saveSupplierProducts(exists ? allSupplierProducts.map((x) => (x.id === entry.id ? entry : x)) : [...allSupplierProducts, entry]); };
  ...
  const validateOrder = (order) => {
    const nextProducts = products.map((p) => {
      ...
      if (!line) return p;
      const unitsReceived = line.crates * line.crateSize;
      const unitCost = line.crateSize > 0 ? Math.round(line.cratePrice / line.crateSize) : p.costPrice;
      return { ...p, stock: p.stock + unitsReceived, costPrice: unitCost };
    });
    const receiveMovements = order.items.map((i) => {
      const unitsReceived = i.crates * i.crateSize;
      const p = nextProducts.find((x) => x.id === i.productId);
      const before = p ? p.stock - unitsReceived : 0;
      return { id: uid(), date: new Date().toISOString(), productId: i.productId, productName: i.productName, type: "livraison", delta: unitsReceived, before, after: before + unitsReceived, author: "Administrateur", note: `${i.crates} casier${i.crates > 1 ? "s" : ""} de ${i.crateSize}` };
    });
    ...
    pushToast("Commande validée — stock et prix de revient mis à jour, dépense enregistrée", "ok");
  };
  ...
  return (
    <div>
        ...
        {suppliers.map((s) => {
          ...
          return editing === s.id ? (
            ...
            <div key={s.id} className="rounded-2xl p-3 border" style={{ borderColor: "var(--line)", background: "var(--card)" }}>
              ...
              <button onClick={() => setCatalogFor(s)} className="gb-focus w-full rounded-xl py-2 text-xs font-semibold mb-2" style={{ background: "var(--paper-dim)" }}>📦 Liste des produits</button>
              <button onClick={() => setOrderFor(s)} className="gb-focus w-full rounded-xl py-2 text-xs font-semibold mb-2" style={{ background: "var(--paper-dim)" }}>📋 Créer un bon de commande</button>
              {supplierOrders.length > 0 && (
                <div className="flex flex-col gap-1.5 pt-1">
                  {pendingCount > 0 && <p className="text-[10px] font-semibold opacity-50 px-0.5">{pendingCount} commande{pendingCount > 1 ? "s" : ""} en attente</p>}
                  {supplierOrders.slice(0, 4).map((o) => (
                    <button
                      key={o.id}
                      onClick={() => o.status === "pending" && setOpenOrder(o)}
                      className="gb-focus rounded-xl px-3 py-2 flex items-center justify-between"
                      style={{ background: "var(--paper-dim)", opacity: o.status === "received" ? 0.7 : 1 }}
                    >
                      <span className="text-[11px]">{new Date(o.date).toLocaleDateString("fr-FR")} · {o.items.length} article{o.items.length > 1 ? "s" : ""}</span>
                      <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full text-white" style={{ background: o.status === "received" ? "#3B6D11" : "var(--cap)" }}>
                        {o.status === "received" ? "REÇUE" : "EN ATTENTE"}
                      </span>
                    </button>
                  ))}
                </div>
      ...
      {orderFor && <PurchaseOrderModal supplier={orderFor} supplierProducts={allSupplierProducts} onCreate={createOrder} onClose={() => setOrderFor(null)} />}
      {openOrder && (
        <PendingOrderModal
          order={openOrder}
...
function AdminScreen({
  ...
  return (
    <div className="px-4 pt-4 pb-28">
      ...
      {section === "fournisseurs" && <SuppliersSection suppliers={suppliers} saveSuppliers={saveSuppliers} expenses={expenses} saveExpenses={saveExpenses} products={products} saveProducts={saveProducts} movements={movements} saveMovements={saveMovements} orders={orders} saveOrders={saveOrders} supplierProducts={supplierProducts} saveSupplierProducts={saveSupplierProducts} pushToast={pushToast} />}
